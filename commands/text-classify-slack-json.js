const { getSlackParentText } = require('../utils/get-slack-parent-thread.js')
...
const { loadTrainingCsv, trainModel, evaluateTrainingData } = require('../utils/read-csv.js');

const textClassifySlackJson = async (trainingcsv, slackInput, outputcsv, probability) => {
  // csv strings
  ...
  console.log("Now reading input file from Slack JSON.")

  const parentThreads = await getSlackParentText(slackInput);
  let parentText = parentThreads.map(i => i.text);
  let cleanedText = cleanText(parentText);
  let obfuscatedText = obfuscateComments(cleanedText);
  ...
  let probabilityHeaders = (probability) ? `,${uniqueCategories}` : '';
...
module.exports = textClassifySlackJson;
